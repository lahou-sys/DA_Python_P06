...
// Functions used to add a info button to open a modal
function add_button_info(element,url) {
    let newButton = document.createElement('a');
    newButton.text = "+Info";
    newButton.classList = 'info_button';
...
// Functions used to add paragraph of the summary of the film
async function add_summary_of_film(element, film_url) {
    let newParagraph = document.createElement('p');
    newParagraph.className = "summaryBestMovie";
    film_description = await get_infos_movie(film_url);
    newParagraph.textContent += film_description.long_description;
...
async function get_infos_movie(film_url) {
    return await get_film_information_from_url(film_url);
}
